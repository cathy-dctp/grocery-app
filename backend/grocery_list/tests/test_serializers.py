...
from grocery_list.models import Category
...
class TestCategorySerializer:
    ...
    def test_category_deserialization(self, db):
        ...
        data = {"name": "Vegetables", "description": "Fresh vegetables and greens"}
...
class TestGroceryListItemSerializer:
    ...
    def test_grocery_list_item_nested_read_only_fields(self, db):
        ...
        category = Category.objects.create(
            name="BeveragesCoffee", description="Coffee drinks"
        )
        item = ItemFactory(name="Coffee", category=category)
        adder = UserFactory(username="adder")

        grocery_list_item = GroceryListItemFactory(
            item=item, added_by=adder, checked_by=None  # Not checked yet
        )
    ...
    def test_grocery_list_item_display_name_with_custom_name(self, db):
        ...
        grocery_list_item = GroceryListItemFactory(
            item=item, custom_name="Organic Red Apples"
        )
        serializer = GroceryListItemSerializer(grocery_list_item)

        assert serializer.data["display_name"] == "Organic Red Apples"
        assert (
            serializer.data["item_name"] == "Apple"
        )  # Original item name still available
    ...
    def test_grocery_list_item_display_name_fallback_to_item_name(self, db):
        ...
        grocery_list_item = GroceryListItemFactory(
            item=item, custom_name=""  # Empty custom name
        )
        serializer = GroceryListItemSerializer(grocery_list_item)
    ...
    def test_grocery_list_item_display_name_fallback_to_item_name_when_none(self, db):
        ...
        # display_name method handles falsy values, including empty string
        item = ItemFactory(name="Orange")
        grocery_list_item = GroceryListItemFactory(
            item=item, custom_name=""  # Use empty string instead of None
        )
    ...
    def test_grocery_list_item_serialization_with_custom_name(self, db):
        ...
        grocery_list_item = GroceryListItemFactory(custom_name="My Special Item")
        serializer = GroceryListItemSerializer(grocery_list_item)
...
class TestGroceryListSimpleSerializer:
    ...
    def test_grocery_list_simple_serialization(self, db):
        ...
        grocery_list = GroceryListFactory(
            name="Weekly Shopping", owner=owner, is_active=True
        )
    ...
    def test_grocery_list_deserialization(self, db):
        ...
        owner = UserFactory()
        data = {"name": "New List", "is_active": True}
