...
class TestCategoryViewSet:
    ...
    def test_list_categories_authenticated(
        self, authenticated_client, category_list_url, db
    ):
    ...
    def test_create_category_duplicate_name(
        ...
        data = {"name": "Existing", "description": "Should fail"}
...
class TestItemViewSet:
    ...
    def test_create_item_invalid_category(
        self, authenticated_client, item_list_url, db
    ):
    ...
    def test_create_item_duplicate_name_in_category(
        ...
        data = {"name": "Existing Item", "category": category.id}

        response = authenticated_client.post(item_list_url, data, format="json")

        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_create_item_same_name_different_category(
        self, authenticated_client, item_list_url, db
    ):
        ...
        data = {"name": "Milk", "category": category2.id}
...
class TestGroceryListViewSet:
    ...
    def test_list_grocery_lists_authenticated(
        self, authenticated_client, grocery_list_url, db
    ):
    ...
    def test_retrieve_own_grocery_list(self, authenticated_client, db):
        ...
        grocery_list = GroceryListFactory(name="Test List", owner=user)
    ...
    def test_retrieve_shared_grocery_list(self, authenticated_client, db):
        ...
        grocery_list = GroceryListFactory(name="Shared List", owner=owner)
    ...
    def test_create_grocery_list_authenticated(
        self, authenticated_client, grocery_list_url, db
    ):
    ...
    def test_create_grocery_list_invalid_data(
        self, authenticated_client, grocery_list_url, db
    ):
    ...
    def test_update_own_grocery_list(self, authenticated_client, db):
        ...
        user = authenticated_client.user
        grocery_list = GroceryListFactory(name="Old Name", owner=user)
    ...
    def test_partial_update_own_grocery_list(self, authenticated_client, db):
        ...
        user = authenticated_client.user
        grocery_list = GroceryListFactory(name="Original Name", owner=user)
    ...
    def test_grocery_lists_ordered_by_updated_date(
        self, authenticated_client, grocery_list_url, db
    ):
...
class TestCustomActions:
    ...
    def test_add_item_to_grocery_list_success(self, authenticated_client, db):
        ...
        grocery_list_item = GroceryListItem.objects.get(
            grocery_list=grocery_list, item=item
        )
    ...
    def test_add_existing_item_creates_new_entry(self, authenticated_client, db):
        """Test that adding existing item creates new entry vs incrementing."""
        user = authenticated_client.user
        category = CategoryFactory()
        ...
        items_in_list = GroceryListItem.objects.filter(
            grocery_list=grocery_list, item=item
        )
    ...
    def test_add_item_nonexistent_item(self, authenticated_client, db):
        ...
        url = f"/api/grocery-lists/{grocery_list.id}/add_item/"
        data = {"item_id": 999}  # Nonexistent item
    ...
    def test_toggle_checked_unchecked_to_checked(self, authenticated_client, db):
        ...
        grocery_list_item = GroceryListItemFactory(
            grocery_list=grocery_list, item=item, added_by=user, is_checked=False
        )
    ...
    def test_toggle_checked_other_user_item_forbidden(self, authenticated_client, db):
        ...
        item = ItemFactory()
        grocery_list_item = GroceryListItemFactory(
            grocery_list=grocery_list, item=item, added_by=other_user
        )
    ...
    def test_toggle_checked_shared_list_item_success(self, authenticated_client, db):
        ...
        item = ItemFactory()
        grocery_list_item = GroceryListItemFactory(
            grocery_list=grocery_list, item=item, added_by=owner, is_checked=False
        )
...
class TestGroceryListItemPatchUpdates:
    ...
    def test_patch_update_custom_name_only(self, authenticated_client, db):
        ...
        data = {"custom_name": "Updated Custom Name"}
    ...
    def test_patch_clear_custom_name(self, authenticated_client, db):
        """Test PATCH can clear custom_name to fall back to item name."""
        user = authenticated_client.user
        grocery_list = GroceryListFactory(owner=user)
        ...
        url = self.grocery_list_item_detail_url(grocery_list_item.pk)
        data = {"custom_name": ""}  # Clear the custom name
        ...
        assert (
            response_data["display_name"] == "Banana"
        )  # Should fall back to item name
...
class TestEnhancedDuplicateItemScenarios:
    ...
    def test_same_item_different_units(self, authenticated_client, db):
        ...
        data1 = {"item_id": item.id, "quantity": 5, "unit": "pieces"}
        response1 = authenticated_client.post(url, data1, format="json")
        assert response1.status_code == status.HTTP_201_CREATED

        # Add apples by weight
        data2 = {"item_id": item.id, "quantity": 2.5, "unit": "lbs"}
    ...
    def test_same_item_different_custom_names(self, authenticated_client, db):
        """Test same item with different custom names creates separate entries."""
        user = authenticated_client.user
        category = CategoryFactory()
    ...
    def test_add_checked_item_again(self, authenticated_client, db):
        ...
        url = f"/api/grocery-lists/{grocery_list.id}/add_item/"
        data = {"item_id": item.id, "quantity": 2, "unit": "loaves"}
    ...
    def test_multiple_quantities_same_item(self, authenticated_client, db):
        ...
        for qty in quantities:
            data = {"item_id": item.id, "quantity": qty}
            response = authenticated_client.post(url, data, format="json")
            assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_duplicate_items_ordering(self, authenticated_client, db):
        """Test duplicate items ordering (unchecked first, then by item name)."""
        user = authenticated_client.user
        grocery_list = GroceryListFactory(owner=user)
        ...
        items = list(
            GroceryListItem.objects.filter(grocery_list=grocery_list, item=item)
        )
